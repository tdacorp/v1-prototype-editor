...
export default async function ClientPage({
	...
	if (prefix === "dev" && type && !variant) {
		...
		// ⚠️ Handle invalid component type gracefully
		if (!componentGroup) {
			return (
				<main className="min-h-screen flex flex-col items-center justify-center bg-gray-50 text-gray-600">
					<h1 className="text-2xl font-semibold mb-2">Unknown Component Type</h1>
					<p>
	...
	if (prefix === "dev" && type && variant) {
		...
		// ⚠️ Handle invalid variant gracefully
		if (!schema) {
			return (
		...
		switch (type) {
			...
			default:
				// TODO: Add more component type renderers here when new components are added
				return (
